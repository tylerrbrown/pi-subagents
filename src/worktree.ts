...
export interface WorktreeCleanupResult {
  /** Whether changes were found, or must conservatively be assumed. */
  hasChanges: boolean;
  /** Branch name if changes were committed. */
  branch?: string;
  /** Worktree path if it was kept. */
  path?: string;
  /** Preservation failure when the worktree had to be retained for recovery. */
  error?: string;
}
...
function safeRepoSubdir(prefix: string): string | undefined {
  if (!prefix) return "";
...
export async function createWorktree(
  ...
  try {
    ...
    if (safeSubdir === undefined) throw new Error("Git worktree prefix escapes the repository");
    subdir = safeSubdir;
  } catch {
    return undefined;
  }
