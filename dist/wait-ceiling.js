 ...
 */
export function normalizeWaitCeilingMs(n) {
    if (n == null || n === 0)
        return undefined;
    return Math.max(MIN_WAIT_CEILING_MS, n);
}
/**
 ...
 */
export function waitWithCeiling(promise, ceilingMs, signal) {
...
export function hasBlockingWait() { return inFlight > 0; }
/** Claim the single blocking-wait slot. `false` means a join is already in flight. */
export function beginBlockingWait() {
    if (inFlight > 0)
        return false;
    inFlight++;
    return true;
...
export function endBlockingWait() {
    if (inFlight > 0)
        inFlight--;
}
/**
 ...
 */
export const PARALLEL_JOIN_REFUSAL = "Refused: a blocking get_subagent_result(wait: true) has already used this turn's wait " +
