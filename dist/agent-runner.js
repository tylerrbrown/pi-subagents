 ...
 */
export function resolveEffectiveMaxTurns(type, explicit) {
...
export function getRememberAgents() { return rememberAgents; }
/** Set whether subagent sessions are persisted by default. */
export function setRememberAgents(b) { rememberAgents = b; }
/**
 ...
 */
export function normalizeRunDeadlineMs(n) {
    if (n == null || n === 0)
        return undefined;
    return Math.max(MIN_RUN_DEADLINE_MS, n);
}
const MIN_RUN_DEADLINE_MS = 1_000;
let defaultRunDeadlineMs;
/** Project default run deadline. undefined = unlimited. */
export function getDefaultRunDeadlineMs() { return defaultRunDeadlineMs; }
/** Set the project default run deadline. undefined or 0 = unlimited. */
export function setDefaultRunDeadlineMs(n) {
...
export function resolveEffectiveRunDeadlineMs(explicit) {
 ...
 */
export function armRunDeadline(deadlineMs, abort) {
    if (deadlineMs == null)
        return { timedOut: () => false, cancel: () => { } };
    let fired = false;
    const timer = setTimeout(() => {
        ...
        catch { /* a half-disposed session must not hide the timeout */ }
    }, deadlineMs);
    timer.unref?.();
    return {
        timedOut: () => fired,
 ...
 */
export function resolveDefaultModel(parentModel, registry, configModel) {
    if (configModel) {
...
export async function runAgent(ctx, type, prompt, options) {
    ...
    const { extNames, narrowing } = parseExtSelectors(options.isolated ? [] : [...(agentConfig?.extSelectors ?? []), ...addedExtSelectors]);
    ...
    const deadline = armRunDeadline(resolveEffectiveRunDeadlineMs(options.runDeadlineMs), () => session.abort());
 ...
 */
export function getAgentConversation(session, maxChars) {
    ...
    let sectionCount = 0;
    let truncated = false;
    const append = (value, start = 0, end = value.length) => {
        const available = contentBudget - length;
        const wanted = end - start;
        if (wanted <= available) {
            if (wanted > 0)
                parts.push(start === 0 && end === value.length ? value : value.slice(start, end));
            length += wanted;
            return true;
        }
        if (available > 0)
            parts.push(value.slice(start, start + available));
        length += Math.max(0, available);
        truncated = true;
        return false;
    };
    const beginSection = (prefix) => {
        if (sectionCount > 0 && !append("\n\n"))
            return false;
        sectionCount++;
        return append(prefix);
    };
    const trimmedBounds = (value) => {
        let start = 0;
    ...
    outer: for (const msg of session.messages) {
        if (msg.role === "user") {
            if (typeof msg.content === "string") {
                ...
                if (start === end)
                    continue;
                if (!beginSection("[User]: ") || !append(msg.content, start, end))
                    break;
            }
            else {
                let began = false;
                for (const content of msg.content) {
                    ...
                    if (start === end)
                        continue;
                    if (!began) {
                        if (!beginSection("[User]: "))
                            break outer;
                        began = true;
                    }
                    else if (!append("\n"))
                        break outer;
                    if (!append(content.text, start, end))
                        break outer;
                }
            }
        }
            ...
            for (const content of msg.content) {
                ...
                if (!beganTools) {
                    ...
                    break outer;
                if (!append(`  Tool: ${call.name ?? call.toolName ?? "unknown"}`))
                    break outer;
            }
        }
        else if (msg.role === "toolResult") {
            let text = "";
            for (const content of msg.content) {
                if (content.type !== "text" || !content.text || text.length > 200)
                    continue;
                if (text)
                    text += "\n";
            ...
            if (!beginSection(`[Tool Result (${msg.toolName})]: `) || !append(preview))
                break;
        }
    }
