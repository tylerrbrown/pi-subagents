...
function getStatusLabel(status, error) {
    switch (status) {
        ...
        case "timeout": return "Timed out (wall-clock deadline)";
        default: return "Done";
    }
}
...
function boundedXmlText(value, maxChars) {
    let escaped = "";
    for (const char of value) {
        ...
        if (escaped.length + next.length > maxChars - 1)
            return escaped + "…";
        escaped += next;
    }
    return escaped;
...
function getRecordOutputFile(record) {
...
function formatTaskNotification(record, showCost = false) {
...
export default function (pi) {
    ...
    pi.registerMessageRenderer("subagent-notification", (message, { expanded }, theme) => {
        ...
        function renderOne(d) {
            const isError = d.status === "error" || d.status === "stopped" || d.status === "aborted" || d.status === "timeout";
    ...
    let claimedNotificationIds = new Set();
    let consumedNotificationIds = new Set();
    const snapshotRetries = new Map();
    let wakeRetryAttempts = 0;
    function cancelNotificationTimer() {
        if (notificationTimer !== undefined)
            clearTimeout(notificationTimer);
        notificationTimer = undefined;
    }
    function invalidateNotificationTimer() {
        cancelNotificationTimer();
        snapshotRetries.clear();
        wakeRetryAttempts = 0;
    }
    function appendNotificationTransition(action, agentIds) {
    ...
    function applyNotificationTransition(action, agentIds) {
        for (const id of agentIds) {
            pendingNotificationIds.delete(id);
            if (action === "wake_claimed")
                claimedNotificationIds.add(id);
            else
                consumedNotificationIds.add(id);
        }
    }
    function snapshotRecord(record, notificationPending) {
        if (record.status === "queued" || record.status === "running" || record.completedAt === undefined)
            return undefined;
    ...
    function appendCompletedRecord(record, notificationPending, force = false) {
        ...
        if (!duplicate || force)
            pi.appendEntry("subagents:record", snapshot);
        persistedRecords.set(record.id, snapshot);
        snapshotRetries.delete(record.id);
        if (!notificationPending)
            pendingNotificationIds.delete(record.id);
        if (!duplicate) {
    ...
    function drainSnapshotPersistenceRetries() {
        const exhausted = [];
        while (snapshotRetries.size > 0)
            exhausted.push(...retrySnapshotPersistence());
        return exhausted;
    }
    function canFlushNotifications() {
        if (!currentCtx)
            return false;
        return (currentCtx.isIdle?.() ?? true) && !(currentCtx.hasPendingMessages?.() ?? false);
    }
    function flushPendingNotifications() {
        if (notificationFlushActive || pendingNotificationIds.size === 0 || !canFlushNotifications())
            return;
        ...
        try {
            ...
            if (rest.length > 0)
                details.others = rest.map(record => buildNotificationDetails(record));
            ...
            if (content.length > MAX_NOTIFICATION_CHARS)
                return;
    ...
    function scheduleIdleNotificationFlush(delay = NUDGE_HOLD_MS) {
        if (notificationTimer !== undefined)
            clearTimeout(notificationTimer);
    ...
    function makeNotificationEligible(records) {
        if (wakeRetryAttempts >= MAX_NOTIFICATION_RETRIES)
            wakeRetryAttempts = 0;
        for (const record of records) {
            if (record.resultConsumed || consumedNotificationIds.has(record.id) || claimedNotificationIds.has(record.id))
                continue;
            pendingNotificationIds.add(record.id);
        }
        if (pendingNotificationIds.size > 0)
            scheduleIdleNotificationFlush();
    }
    function consumeNotification(id) {
    ...
    // ---- Group join manager ----
    const groupJoin = new GroupJoinManager((records, _partial) => {
        for (const record of records) {
            agentActivity.delete(record.id);
            widget.markFinished(record.id);
            fleet.onAgentFinished(record.id);
    ...
    function finishPersistedCompletion(record) {
        ...
        if (groupJoin.onAgentComplete(record) === "pass")
    ...
    const manager = new AgentManager((record) => {
        ...
        if (record.parentAgentId || manager.getRecord(record.id) !== record)
            return;
        noteWaitOutcome("settled");
        // Emit lifecycle event based on terminal status
        const isError = record.status === "error" || record.status === "stopped" || record.status === "aborted" || record.status === "timeout";
     ...
     */
    const renderTombstone = (entry) => {
        const head = `Agent: ${entry.id}\n` +
        ...
        if (entry.status === "error")
            return head + `Error: ${entry.error ?? "unknown"}`;
    ...
    const resolvePersistedAgentRef = (ref) => {
        const byId = persistedRecords.get(ref);
        if (byId)
            return byId;
        const normalized = ref.replace(/^@/, "").toLowerCase();
        return [...persistedRecords.values()].find(record => record.handles.handle?.toLowerCase() === normalized || record.handles.alias?.toLowerCase() === normalized);
    };
    const registryEntry = {
    ...
    pi.on("session_before_switch", async () => {
        ...
        sessionSwitchPromise = (async () => {
            ...
            if (exhaustedSnapshots.length > 0) {
                console.warn(`[pi-subagents] Failed to persist ${exhaustedSnapshots.length} agent completion record(s) `
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
    ...
    const agentTool = defineTool({
        ...
        parameters: Type.Object({
            ...
            deadline_ms: Type.Optional(Type.Number({
                description: "Wall-clock ceiling for this run, in milliseconds. The run ends in a `timeout` status with its " +
                    "partial output kept. Use it for work that can stall on a provider rather than on turns. " +
                    "Omit for the project default; use 0 to make this run unlimited.",
                minimum: 0,
            })),
        ...
        renderResult(result, { expanded, isPartial }, theme, renderContext) {
            ...
            if (details.status !== "error" && details.status !== "aborted" && details.status !== "timeout") {
                return new Text(text, 0, 0);
            }
            ...
            if (details.status === "error") {
                ...
                line += "\n" + theme.fg("error", "  ⎿  Timed out (wall-clock deadline; partial output may be incomplete)");
            }
            else {
        ...
        execute: async (toolCallId, params, signal, onUpdate, ctx) => {
            ...
            if (record.status === "timeout") {
                return textResult(`${fallbackNote}Agent timed out (wall-clock deadline).${partialOutputSuffix(record)}`, details);
            }
            ...
            return textResult(`${fallbackNote}Agent completed in ${formatMs(durationMs)} (${statsParts.join(", ")})${getForegroundOutcomeNote(record.status, record.stopReason)}.\n\n` +
    ...
    registerToolReportingUsage(defineTool({
        ...
        execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
            ...
            const record = liveRecord ?? resolvePersistedAgentRef(params.agent_id);
            if (!record) {
                const remains = manager.getTombstone(params.agent_id);
                if (remains)
                    return textResult(renderTombstone(remains));
                return textResult(`Agent not found: "${params.agent_id}". It may have been cleaned up.`);
            }
            ...
            if (liveRecord && params.wait && (liveRecord.status === "running" || liveRecord.status === "queued")) {
                if (shouldRefuseRepeatJoin())
                    return textResult(PARALLEL_JOIN_REFUSAL);
                if (!beginBlockingWait())
                    return textResult(PARALLEL_JOIN_REFUSAL);
                const ceilingMs = getWaitCeilingMs();
                const startedWaitingAt = Date.now();
                const remainingMs = () => ceilingMs == null ? undefined : Math.max(0, ceilingMs - (Date.now() - startedWaitingAt));
                try {
            ...
            if (needsConsumption) {
                ...
                if (liveRecord)
                    liveRecord.resultConsumed = true;
            }
            return result;
        },
    }));
